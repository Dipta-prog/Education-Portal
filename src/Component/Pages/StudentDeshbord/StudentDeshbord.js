...
const StudentDeshbord = () => {
  ...
  const fetchStudentDashboardData = async () => {
    const res = await fetch(
      "https://education-portal-1.herokuapp.com/students"
    );
    let data = await res.json();
    data = data.rasult;
    console.log(
      "🚀 ~ file: StudentDeshbord.js ~ line 37 ~ loadStudentDashboardData ~ data",
      data
    );
    const filterRoleAndLoginUser = data.filter(
      (course) => course.loggedinUser.email === loggedinUser.email
    );
    setStudentDeshboardData(filterRoleAndLoginUser);
  };
  console.log("studentDeshboardData", studentDeshboardData);
  ////////////////
  // const allls = studentDeshboardData.map(
  //   (all) => all.studentSelactedCourse.subject
  // );
  ...
  return loggedinUser.role === "student" ? (
    ...
    <div className="m-5 p-5">
      <div className="text-danger text-center p-5 m-5">
        {" "}
        <h4 className="text-danger p-5 m-5">
